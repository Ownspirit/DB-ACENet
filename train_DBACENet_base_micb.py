...
from model.DBACENet_base_micb import DBACENet
...
def parse_args():
    ...
    parser.add_argument('--fusion_depth', default=8, type=int, help="Model Fusion STE and TTE depth")
...
def evaluate(args, model, test_loader, datareader, device):
    ...
    with torch.no_grad():
        for x, y, feature, flip_feature in tqdm(test_loader):#
            x, y, feature, flip_feature = x.to(device), y.to(device), feature.to(device), flip_feature.to(device)

            if args.flip:
                batch_input_flip = flip_data(x)
                predicted_3d_pos_1 = model(x, feature)
                predicted_3d_pos_flip = model(batch_input_flip, flip_feature)
...
def train(args, opts):
    ...
    # for name, param in model.named_parameters():
    #     print(f"Layer: {name} | Size: {param.size()} | Requires Grad: {param.requires_grad} | Num Params: {param.numel()}")
    ...
    if opts.checkpoint:
        checkpoint_path = os.path.join(opts.checkpoint, opts.checkpoint_file if opts.checkpoint_file else "best_epoch.pth.tr")
