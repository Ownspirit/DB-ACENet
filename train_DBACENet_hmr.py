...
from data.reader.motion_dataset_rgb import MotionDataset3D
...
# from model.Fusion_k_win_AGformer import Fusion
from model.DBACENet_hmr import DBACENet
...
def parse_args():
    ...
    parser.add_argument('--num-cpus', default=20, type=int, help='Number of CPU cores')
    parser.add_argument('--use-wandb', action='store_true')
    parser.add_argument('--wandb-name', default=None, type=str)
    parser.add_argument('--wandb-run-id', default=None, type=str)
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--eval-only', action='store_true')
    parser.add_argument('--fusion_depth', default=1, type=int, help="Model Fusion STE and TTE depth")
    parser.add_argument('--fusion_dim', default=128, type=int, help="Model Fusion Hidden dim")
    parser.add_argument('--fusion_method', default='GCN', type=str, help="Model Fusion Method in [CAT, ADD, MLP, CA, GCN]")
    parser.add_argument('--lifting_type', default="MotionAGFormer", help="choose lifting model")
...
def train_one_epoch(args, model, train_loader, optimizer, device, losses):
    model.train()
    for x, y, feature in tqdm(train_loader):
        batch_size = x.shape[0]
        #x, y, feature = x.to(device), y.to(device), feature.to(device)
        y, feature =  y.to(device), feature.to(device)
...
def evaluate(args, model, test_loader, datareader, device):
    ...
    model.eval()
    with torch.no_grad():
        for x, y, feature, flip_feature in tqdm(test_loader):#
            y, feature, flip_feature =  y.to(device), feature.to(device), flip_feature.to(device)

            if args.flip:
                #batch_input_flip = flip_data(x)
                predicted_3d_pos_1 = model(feature)
                predicted_3d_pos_flip = model(flip_feature)
                predicted_3d_pos_2 = flip_data(predicted_3d_pos_flip)  # Flip back
                predicted_3d_pos = (predicted_3d_pos_1 + predicted_3d_pos_2) / 2
            else:
                predicted_3d_pos = model(feature)
            if args.root_rel:
    ...
    for action in action_names:
        ...
        print(action, "MPJPE:", np.mean(results[action]), "PA-MPJPE:", np.mean(results_procrustes[action]), "MPJVE", np.mean(results_accelaration[action]))
...
def load_partial_weights(model, checkpoint, freeze_loaded=True):
...
def copy_some_para(args, opts):
    args.fusion_depth = opts.fusion_depth
    args.fusion_dim = opts.fusion_dim
    args.fusion_method = opts.fusion_method
    args.lifting_type = opts.lifting_type
    return args, opts

def train(args, opts):
    args, opts = copy_some_para(args, opts)
    print_args(args)
    ...
    datareader = DataReaderH36M(n_frames=args.n_frames, sample_stride=1,
                                ...
                                dt_root='/root/autodl-tmp', dt_file=args.dt_file)  # Used for H36m evaluation

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    # model = load_model(args)
    # model = Fusion(args)
    """
        Load Train_DBACENet model
    """
    model = DBACENet(args)
    ...
    # hmr_checkpoint_path = "/root/autodl-tmp/hmr.bin"
    ...
    # hmr_dict = {} 
    # d2_dict = {}
    # for old_name, param in hmr_checkpoint['model_pos'].items():
    #     new_name = "module.rgb_embed" + old_name[6:]
    #     hmr_dict[new_name] = param
    # for old_name, param in checkpoint_2d['model'].items():
    #     new_name = "module.pose_embed" + old_name[6:]
    #     d2_dict[new_name] = param
    # model = load_partial_weights(model, hmr_dict)
    # model = load_partial_weights(model, d2_dict, freeze_loaded=False)
    ...
    if opts.checkpoint:
        checkpoint_path = os.path.join(opts.checkpoint, opts.checkpoint_file if opts.checkpoint_file else "best_hmr.pth.tr")
        if os.path.exists(checkpoint_path):
            checkpoint = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)
            model.load_state_dict(checkpoint['model'], strict=True)
            print(checkpoint['min_mpjpe'])
    ...
    checkpoint_path_best = os.path.join(opts.new_checkpoint, 'best_hmr.pth.tr')
