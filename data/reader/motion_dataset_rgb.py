...
class Fusion(Dataset):
    def __init__(self, opt, train=True):
        ...
        if self.train:
            ...
            self.poses_test, self.poses_test_2d, self.valid_frame, self.feature, self.flip_feature = self.prepare_data(opt.data_root, train=False)
            self.generator = ChunkedGenerator(opt.test_batch_size, None, self.poses_test,
                                                ...
                                                feature=self.feature, flip_feature=self.flip_feature)
    ...
    def prepare_data(self, path, train=True):
        ...
        if train == True:
            ...
            feature_file = "3dhp_train_feature.pkl" if self.train else "3dhp_test_feature.pkl"
            flip_feature_file = "flip_3dhp_train_feature.pkl" if self.train else "flip_3dhp_test_feature.pkl"
            feature_data = joblib.load(os.path.join(path, feature_file))
            flip_feature_data = joblib.load(os.path.join(path, flip_feature_file))
            ...
            return out_poses_3d, out_poses_2d, valid_frame, feature_data, flip_feature_data
    ...
    def __getitem__(self, index):
        ...
        cam, gt_3D, input_2D, seq, _, _, feature = self.generator.get_batch(seq_name, start_3d, end_3d, flip, reverse)

        if self.train == False and self.test_aug:
            _, _, input_2D_aug, _, _,_, flip_feature = self.generator.get_batch(seq_name, start_3d, end_3d, flip=True, reverse=reverse)
            input_2D = np.concatenate((np.expand_dims(input_2D,axis=0),np.expand_dims(input_2D_aug,axis=0)),0)
            # feature = np.concatenate((np.expand_dims(feature, axis=0), np.expand_dims(flip_feature, axis=0)), 0)
        ...
        if self.train == True:
            ...
            return cam, gt_3D, input_2D_update, seq, scale, bb_box, feature, flip_feature
            

class MPI3DHP(Dataset):
    def __init__(self, args, train=True):
        self.train = train
        self.poses_3d, self.poses_2d, self.feature, self.flip_feature, self.poses_3d_valid_frames, self.seq_names = self.prepare_data(args)
    ...
    def prepare_data(self, args):
        ...
        feature_file = "3dhp_train_feature.pkl" if self.train else "3dhp_test_feature.pkl"
        flip_feature_file = "flip_3dhp_train_feature.pkl" if self.train else "flip_3dhp_test_feature.pkl"
        feature_data = joblib.load(os.path.join(args.data_root,feature_file))
        flip_feature_data = joblib.load(os.path.join(args.data_root,flip_feature_file))
        ...
        for seq in data.keys():
            if self.train:
                for cam in data[seq][0].keys():
                    ...
                    feature_cam = feature_data[sub][Sq]['video_'+cam]
                    flip_feature_cam = flip_feature_data[sub][Sq]['video_' + cam]
                    data_3d_partitioned, data_2d_partitioned, _ = self.extract_poses(anim, seq, n_frames, stride)
                    feature_partitioned, flip_feature_partitioned = self.extract_features(feature_cam, flip_feature_cam, n_frames, stride)
                    feature.extend(feature_partitioned)
                    flip_feature.extend(flip_feature_partitioned)
                    poses_3d.extend(data_3d_partitioned)
                    poses_2d.extend(data_2d_partitioned)
            else:
                anim = data[seq]
                valid_frames = anim['valid']
                feature_video = feature_data[seq]
                flip_feature_video  = flip_feature_data[seq]
                ...
                feature_partitioned, flip_feature_partitioned = self.extract_features(feature_video, flip_feature_video, n_frames, stride, valid_frames)
                feature.extend(feature_partitioned)
                flip_feature.extend(flip_feature_partitioned)    
                poses_3d.extend(data_3d_partitioned)
                poses_2d.extend(data_2d_partitioned)
        ...
        # flip_feature = np.concatenate(flip_feature, axis=0)
        if len(poses_3d_valid_frames) > 0:
            poses_3d_valid_frames = np.concatenate(poses_3d_valid_frames, axis=0)

        return poses_3d, poses_2d, feature, flip_feature, poses_3d_valid_frames, seq_names

    def __len__(self):
        return self.poses_3d.shape[0]

    def extract_features(self, feature, flip_feature, n_frames, stride, valid_frames=None):
        feature_partitioned, _ = self.partition(feature, clip_length=n_frames, stride=stride, valid_frames=valid_frames)
        flip_feature_partitioned, _ = self.partition(flip_feature, clip_length=n_frames, stride=stride, valid_frames=valid_frames)
        return feature_partitioned, flip_feature_partitioned
    ...
    def __getitem__(self, index):
        ...
        feature = self.feature[index].squeeze()
        flip_feature = self.flip_feature[index].squeeze()
        ...
        if not self.train:
            ...
            return torch.FloatTensor(pose_2d), torch.FloatTensor(pose_3d_normalized), torch.FloatTensor(pose_3d), \
                   torch.FloatTensor(feature), torch.FloatTensor(flip_feature), torch.IntTensor(valid_frames), seq_name
        ...
        return torch.FloatTensor(pose_2d), torch.FloatTensor(pose_3d_normalized), torch.FloatTensor(feature)
          
    
class MotionDataset3D(Dataset):
    def __init__(self, args, subset_list, data_split, return_stats=False, type="sh"):
        """
        ...
        if self.type == "sh":
            ...
            for sub in subset_list:
                self.subset_list.append(sub + '-gt')
    ...
    def __getitem__(self, idx):
        ...
        motion_feature = motion_file['feature']
        motion_flip_feature = motion_file['flip_feature']
        ...
        if self.data_split == 'train':
            if self.flip and random.random() > 0.5:
                ...
                motion_feature = motion_flip_feature

        elif self.data_split == 'test':
            if self.return_stats:
                return torch.FloatTensor(motion_2d), torch.FloatTensor(motion_3d), motion_file['mean'], motion_file['std'], torch.FloatTensor(motion_feature), torch.FloatTensor(motion_flip_feature)
            else:
                return torch.FloatTensor(motion_2d), torch.FloatTensor(motion_3d), torch.FloatTensor(motion_feature), torch.FloatTensor(motion_flip_feature)


        if self.return_stats:
            return torch.FloatTensor(motion_2d), torch.FloatTensor(motion_3d), motion_file['mean'], motion_file['std'], torch.FloatTensor(motion_feature)
        else:
            return torch.FloatTensor(motion_2d), torch.FloatTensor(motion_3d), torch.FloatTensor(motion_feature)
