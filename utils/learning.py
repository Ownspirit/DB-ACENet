...
from model.modules.DSTFormer import DSTformer
...
def load_model(args, node_flags=False):
    act_mapper = {
        "gelu": nn.GELU,
        'relu': nn.ReLU
    }
    ...
    if args.lifting_type == 'MotionAGFormer':
        model = MotionAGFormer(n_layers= args.n_layers,
                               ...
                               num_joints=18 if node_flags else args.num_joints,
    ...
    # elif args.model_name == "DSTFormer":
    elif args.lifting_type == 'MotionBERT':
        model = DSTformer(dim_in=3, dim_out=3, dim_feat=args.dim_feat, dim_rep=args.dim_rep, 
                                   depth=8, num_heads=args.num_heads, mlp_ratio=args.mlp_ratio, norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                                   maxlen=args.maxlen, num_joints=args.num_joints)        
    # elif args.model_name == "MixSTE":
    elif args.lifting_type == 'MixSTE':
        from model.modules.MixSTE import MixSTE2 
        model = MixSTE2(num_frame=args.maxlen, num_joints=18 if node_flags else args.num_joints, in_chans=3, embed_dim_ratio=128, depth=8,
