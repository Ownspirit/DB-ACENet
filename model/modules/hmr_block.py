...
class Attention(nn.Module):
    ...
    def forward(self, x, vis=False):
        ...
        attn = attn.softmax(dim=-1)# 等价于 nn.Softmax(dim=-1)(attn)
...
class Block(nn.Module):
    ...
    def forward(self, x, vis=False):
        x = x + self.drop_path(self.attn(self.norm1(x), vis=vis)) # x.shape: [B, T, dim]
        x = x + self.drop_path(self.mlp(self.norm2(x))) # x.shape: [B, T, dim]
